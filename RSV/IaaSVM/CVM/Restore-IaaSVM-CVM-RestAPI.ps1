...
try {
    ...
    if ($rpResponse.value -and $rpResponse.value.Count -gt 0) {
        ...
        foreach ($rp in $rpResponse.value) {
            ...
            $rpSecurityType = if ($rp.properties.securityType) { $rp.properties.securityType } else { "N/A" }
            ...
            Write-Host "      Security:     $rpSecurityType" -ForegroundColor Gray
            Write-Host ""
            $index++
        }
        ...
        $selectedRpSecurityType = if ($selectedRecoveryPoint.properties.securityType) { $selectedRecoveryPoint.properties.securityType } else { "N/A" }
        ...
        Write-Host "  Security Type: $selectedRpSecurityType" -ForegroundColor Green
    } else {
...
# --- CVM: Optional Secured VM OS Disk Encryption Set ID ---

$securedVMOsDiskEncryptionSetId = $null

Write-Host ""
Write-Host "  Confidential VM (CVM) - OS Disk Encryption Set ID (optional):" -ForegroundColor Cyan
Write-Host "  (Only required for Confidential VMs with CMK. Leave empty to skip.)" -ForegroundColor Gray
Write-Host "  Format: /subscriptions/{subId}/resourceGroups/{rg}/providers/Microsoft.Compute/diskEncryptionSets/{desName}" -ForegroundColor Gray
$securedVMOsDiskEncryptionSetId = Read-Host "    Enter Disk Encryption Set ID"
if (-not [string]::IsNullOrWhiteSpace($securedVMOsDiskEncryptionSetId)) {
    Write-Host "  CVM Disk Encryption Set: $securedVMOsDiskEncryptionSetId" -ForegroundColor Gray
} else {
    Write-Host "  Skipping CVM configuration (not a Confidential VM)" -ForegroundColor Gray
    $securedVMOsDiskEncryptionSetId = $null
}
...
switch ($recoveryType) {
    "RestoreDisks" {
        ...
        if ($securedVMOsDiskEncryptionSetId) {
            $requestProperties.securedVmDetails = @{
                securedVMOsDiskEncryptionSetId = $securedVMOsDiskEncryptionSetId
            }
            Write-Host "  CVM: securedVmDetails added to restore request" -ForegroundColor Cyan
        }
...
if ($requestProperties.securedVmDetails) {
    $desId = if ($requestProperties.securedVmDetails.securedVMOsDiskEncryptionSetId) { $requestProperties.securedVmDetails.securedVMOsDiskEncryptionSetId } else { "(not set)" }
    Write-Host "  CVM DES ID:        $desId" -ForegroundColor Gray
}
if ($requestProperties.preferredRecoveryPointTier) {
    Write-Host "  Preferred RP Tier: $($requestProperties.preferredRecoveryPointTier)" -ForegroundColor Gray
}
