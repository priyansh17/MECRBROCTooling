<#
.SYNOPSIS
    Stops protection (with retain data) for SAP HANA databases on an Azure Linux VM
    and optionally unregisters the VM from a Recovery Services Vault using REST API.

.DESCRIPTION
    This script manages backup protection for SAP HANA databases on Azure Linux VMs
    ...
    - Lists all protected SAP HANA databases/instances on a VM
    - Stops protection while retaining existing recovery points
    - Optionally prompts to unregister after all datasources are stopped

    MODE 2: Full Unregistration (-Unregister)
    - Stops protection with retain data for ALL active datasources
    ...
    - All datasources on the VM are processed (cannot target individual DBs)
    - The -StopAll behavior is implied; -DatabaseName is ignored

    Supported SAP HANA item types:
    - SAPHanaDatabase    (individual HANA databases, e.g. SYSTEMDB, tenant DBs)
    - SAPHanaDBInstance   (HANA DB instances for HSR / System Replication scenarios)
    ...
    - SAP HANA databases must be in Protected/IRPending/ProtectionStopped state
...
.PARAMETER DatabaseName
    The name of a specific SAP HANA database to stop protection for.
    Only used when -Unregister is NOT specified.
    If omitted, ALL protected datasources on the VM will be listed and
    you can choose to stop protection for all or select one.
    Ignored when -Unregister is specified (all datasources must be processed).

.PARAMETER Unregister
    When specified, stops protection for ALL datasources with retain data,
    then unregisters the VM container from the vault.
    Recovery points are preserved through the stop-with-retain mechanism.

.PARAMETER StopAll
    When specified without -Unregister, stops protection for ALL protected SAP HANA
    datasources on the VM without prompting for individual selection.
    When -Unregister is specified, -StopAll is implied and this switch is ignored.

.PARAMETER SkipConfirmation
    Skip confirmation prompts. Use this for automation/scripting.

.EXAMPLE
    # Stop protection for a specific HANA database (retain data, no unregister)
    .\Unregister-SAPHanaVM-FromVault.ps1 -VaultSubscriptionId "xxxx" -VaultResourceGroup "rg-vault" `
...
.EXAMPLE
    ...
    .\Unregister-SAPHanaVM-FromVault.ps1 -VaultSubscriptionId "xxxx" -VaultResourceGroup "rg-vault" `
        -VaultName "myVault" -VMResourceGroup "rg-hana" -VMName "hana-vm-01" -Unregister

.EXAMPLE
    # Stop protection for ALL HANA datasources without unregistering
    .\Unregister-SAPHanaVM-FromVault.ps1 -VaultSubscriptionId "xxxx" -VaultResourceGroup "rg-vault" `
        -VaultName "myVault" -VMResourceGroup "rg-hana" -VMName "hana-vm-01" -StopAll

.EXAMPLE
    # Interactive mode - lists datasources, prompts for selection, then optionally unregisters
    .\Unregister-SAPHanaVM-FromVault.ps1 -VaultSubscriptionId "xxxx" -VaultResourceGroup "rg-vault" `
...
.NOTES
    ...
    Date: March 13, 2026
    Reference: https://learn.microsoft.com/en-us/azure/backup/sap-hana-db-manage
...
param(
    ...
    [Parameter(Mandatory = $false, HelpMessage = "Name of a specific SAP HANA database to stop protection for. Ignored when -Unregister is specified.")]
    [string]$DatabaseName,

    [Parameter(Mandatory = $false, HelpMessage = "Unregister the VM container after stopping protection. Processes ALL datasources. Recovery points are preserved.")]
    [switch]$Unregister,

    [Parameter(Mandatory = $false, HelpMessage = "Stop protection for ALL protected SAP HANA datasources on the VM without prompting. Implied when -Unregister is used.")]
    [switch]$StopAll,
...
# ============================================================================
# HELPER FUNCTION: Determine the protectedItemType for a HANA datasource
# ============================================================================

function Get-HanaProtectedItemType {
    param(
        [object]$ProtectedItem
    )

    # Determine from the item's own protectedItemType or from the ID path
    $itemType = $ProtectedItem.properties.protectedItemType

    if ($itemType) {
        return $itemType
    }

    # Fallback: infer from the item ID
    $itemId = $ProtectedItem.id
    if ($itemId -match "SAPHanaDBInstance") {
        return "AzureVmWorkloadSAPHanaDBInstance"
    }

    # Default to SAPHanaDatabase
    return "AzureVmWorkloadSAPHanaDatabase"
}

# ============================================================================
# HELPER FUNCTION: Stop protection with retain data for a single datasource
# ============================================================================

function Stop-HanaDatasourceProtection {
    param(
        [object]$ProtectedItem,
        [hashtable]$Headers,
        [string]$ApiVersion
    ...
    $dsFriendlyName = $ProtectedItem.properties.friendlyName
    $currentState = $ProtectedItem.properties.protectionState

    # Skip if already stopped
    if ($currentState -eq "ProtectionStopped") {
        Write-Host "    SKIPPED: '$dsFriendlyName' - protection already stopped" -ForegroundColor Yellow
        return $true
    }

    Write-Host "    Stopping protection for '$dsFriendlyName'..." -ForegroundColor Cyan
    ...
    # Determine the correct protectedItemType for this datasource
    $protectedItemType = Get-HanaProtectedItemType -ProtectedItem $ProtectedItem
    ...
    try {
        ...
        if ($statusCode -eq 200) {
            Write-Host "    SUCCESS: Protection stopped for '$dsFriendlyName' (200 OK)" -ForegroundColor Green
            return $true
            ...
            if ($trackingUrl) {
                ...
                while (-not $completed -and $retryCount -lt $maxRetries) {
                    ...
                    try {
                        ...
                        if ($opStatus -eq "Succeeded") {
                            $completed = $true
                            Write-Host "    SUCCESS: Protection stopped for '$dsFriendlyName'" -ForegroundColor Green
                        } else {
                            $retryCount++
                ...
                if (-not $completed) {
                    Write-Host "    WARNING: Operation timed out for '$dsFriendlyName'. Check Azure Portal." -ForegroundColor Yellow
                }
                return $completed
            }
        }
        ...
        if ($statusCode -eq 202) {
            ...
            Write-Host "    ERROR: Failed to stop protection for '$dsFriendlyName'" -ForegroundColor Red
...
Write-Host "  SAP HANA on Azure VM - Stop Protection & Unregister" -ForegroundColor Cyan
...
if ($Unregister) {
    ...
    Write-Host "  Target Datasources:  ALL (required for unregistration)" -ForegroundColor Yellow
    ...
    if (-not [string]::IsNullOrWhiteSpace($DatabaseName)) {
        ...
        Write-Host "  Target Datasources:  ALL (stop all protected items)" -ForegroundColor Yellow
    } else {
...
# ============================================================================
# STEP 1: LIST ALL PROTECTED SAP HANA DATASOURCES ON THE VM
# ============================================================================

Write-Host ""
Write-Host "STEP 1: Listing Protected SAP HANA Datasources on VM" -ForegroundColor Yellow
Write-Host "------------------------------------------------------" -ForegroundColor Yellow
Write-Host ""

# Query for both SAPHanaDatabase and SAPHanaDBInstance item types
$hanaItemTypes = @("SAPHanaDatabase", "SAPHanaDBInstance")
$allProtectedItems = @()
$vmProtectedDS = @()
$containerName = $null

foreach ($hanaItemType in $hanaItemTypes) {
    $protectedItemsUri = "https://management.azure.com/subscriptions/$vaultSubscriptionId/resourceGroups/$vaultResourceGroup/providers/Microsoft.RecoveryServices/vaults/$vaultName/backupProtectedItems?api-version=$apiVersion&`$filter=backupManagementType eq 'AzureWorkload' and itemType eq '$hanaItemType'"

    try {
        Write-Host "Querying for protected $hanaItemType items..." -ForegroundColor Cyan
        ...
        Write-Host "  WARNING: Failed to query $hanaItemType items: $($_.Exception.Message)" -ForegroundColor Yellow
    }
}
...
$vmProtectedDS = $allProtectedItems | Where-Object {
...
if ($vmProtectedDS.Count -gt 0) {
    # Extract container name from the first item
    if ($vmProtectedDS[0].properties.containerName) {
        $containerName = $vmProtectedDS[0].properties.containerName
    } else {
        # Parse from ID
        $idMatch = $vmProtectedDS[0].id -match "/protectionContainers/([^/]+)/"
        if ($idMatch) { $containerName = $Matches[1] }
    }

    Write-Host "  Found $($vmProtectedDS.Count) protected SAP HANA datasource(s) on VM '$vmName'" -ForegroundColor Green
    Write-Host "  Container: $containerName" -ForegroundColor Gray
    Write-Host ""

    $dsIdx = 1
    foreach ($ds in $vmProtectedDS) {
        $state = $ds.properties.protectionState
        $policy = $ds.properties.policyName
        $lastBackup = $ds.properties.lastBackupTime
        $itemType = $ds.properties.protectedItemType
        $parentName = $ds.properties.parentName
        $stateColor = if ($state -eq "ProtectionStopped") { "Yellow" } else { "White" }

        Write-Host "  [$dsIdx] $($ds.properties.friendlyName)" -ForegroundColor $stateColor
        Write-Host "       HANA Instance:  $parentName" -ForegroundColor Gray
        Write-Host "       Item Type:      $itemType" -ForegroundColor Gray
    ...
    Write-Host "  No protected SAP HANA datasources found on VM '$vmName'." -ForegroundColor Yellow
...
#   Step 2: Stop protection with retain data for ALL active datasources
...
if ($Unregister) {
    ...
    # ------------------------------------------------------------------
    if ($vmProtectedDS.Count -eq 0 -and -not [string]::IsNullOrWhiteSpace($containerName)) {
        Write-Host "  Re-querying protected items using container name..." -ForegroundColor Cyan
        try {
            $allItems2 = @()
            foreach ($hanaItemType in $hanaItemTypes) {
                $reQueryUri = "https://management.azure.com/subscriptions/$vaultSubscriptionId/resourceGroups/$vaultResourceGroup/providers/Microsoft.RecoveryServices/vaults/$vaultName/backupProtectedItems?api-version=$apiVersion&`$filter=backupManagementType eq 'AzureWorkload' and itemType eq '$hanaItemType'"
                $curUri = $reQueryUri
                while ($curUri) {
            ...
            $vmProtectedDS = $allItems2 | Where-Object {
            ...
            if ($vmProtectedDS.Count -gt 0) {
                Write-Host "  Found $($vmProtectedDS.Count) protected datasource(s) via container name match." -ForegroundColor Green
                Write-Host ""
                foreach ($ds in $vmProtectedDS) {
                    Write-Host "    - $($ds.properties.friendlyName) [$($ds.properties.protectedItemType)] (State: $($ds.properties.protectionState))" -ForegroundColor White
                }
                Write-Host ""
            }
        } catch {
    ...
    # ==================================================================
    Write-Host ""
    Write-Host "STEP 2: Stopping Protection (Retain Data) for All Datasources" -ForegroundColor Yellow
    Write-Host "----------------------------------------------------------------" -ForegroundColor Yellow
    Write-Host ""

    # Count active datasources that need stopping
    $activeDSForStop = $vmProtectedDS | Where-Object { $_.properties.protectionState -ne "ProtectionStopped" }
    $alreadyStoppedCount = $vmProtectedDS.Count - $activeDSForStop.Count

    if ($activeDSForStop.Count -gt 0 -and -not $SkipConfirmation) {
        Write-Host "  The following $($activeDSForStop.Count) datasource(s) will have protection STOPPED (data retained):" -ForegroundColor Magenta
        foreach ($ads in $activeDSForStop) {
            Write-Host "    - $($ads.properties.friendlyName) [$($ads.properties.protectedItemType)] (State: $($ads.properties.protectionState))" -ForegroundColor White
        }
        if ($alreadyStoppedCount -gt 0) {
            Write-Host "  ($alreadyStoppedCount datasource(s) already stopped - will be skipped)" -ForegroundColor Gray
        }
        Write-Host ""
    ...
    if ($vmProtectedDS.Count -eq 0) {
        Write-Host "  No protected items to stop. Proceeding to container unregistration." -ForegroundColor Yellow
    } else {
        Write-Host "  Stopping protection for $($vmProtectedDS.Count) datasource(s) with retain data..." -ForegroundColor Cyan
        Write-Host ""

        foreach ($ds in $vmProtectedDS) {
            $currentState = $ds.properties.protectionState
            if ($currentState -eq "ProtectionStopped") {
                Write-Host "    SKIPPED: '$($ds.properties.friendlyName)' - protection already stopped" -ForegroundColor Yellow
                $stopSkipCount++
                continue
            }

            $result = Stop-HanaDatasourceProtection -ProtectedItem $ds -Headers $headers -ApiVersion $apiVersion
    ...
    if ($unregisterSucceeded) {
        ...
        Write-Host "    SAP HANA VM UNREGISTERED SUCCESSFULLY!" -ForegroundColor Green
        Write-Host "  ==========================================================" -ForegroundColor Green
    } else {
        Write-Host "  ==========================================================" -ForegroundColor Yellow
    ...
    Write-Host "    Datasources stopped:    $stopSuccessCount" -ForegroundColor White
    Write-Host "    Already stopped:        $stopSkipCount" -ForegroundColor White
    Write-Host "    Failed to stop:         $stopFailCount" -ForegroundColor White
...
if ($vmProtectedDS.Count -gt 0) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($DatabaseName)) {
        ...
        # Stop all
        $dsToStop = $vmProtectedDS
        Write-Host "  -StopAll specified. Stopping protection for all $($dsToStop.Count) datasource(s)..." -ForegroundColor Cyan
    } else {
        # Interactive selection
        $activeDS = $vmProtectedDS | Where-Object { $_.properties.protectionState -ne "ProtectionStopped" }

        if ($activeDS.Count -eq 0) {
            Write-Host "  All datasources already have protection stopped." -ForegroundColor Green
        } else {
            Write-Host "  Select datasource(s) to stop protection:" -ForegroundColor Cyan
            Write-Host "    [A] All datasources - $($activeDS.Count) active" -ForegroundColor White

            $idx = 1
            foreach ($ds in $activeDS) {
                Write-Host "    [$idx] $($ds.properties.friendlyName) [$($ds.properties.protectedItemType)] (State: $($ds.properties.protectionState))" -ForegroundColor White
                $idx++
            }
            Write-Host ""
            ...
            if ([string]::IsNullOrWhiteSpace($choice) -or $choice -ieq 'A') {
                ...
                $selIdx = [int]$choice - 1
                if ($selIdx -ge 0 -and $selIdx -lt $activeDS.Count) {
                    $dsToStop = @($activeDS[$selIdx])
                } else {
    ...
    # Execute stop protection for each selected datasource
    foreach ($ds in $dsToStop) {
        $result = Stop-HanaDatasourceProtection -ProtectedItem $ds -Headers $headers -ApiVersion $apiVersion
...
# ============================================================================
# CHECK IF ALL DATASOURCES ARE NOW STOPPED - PROMPT FOR UNREGISTER
# ============================================================================

$runUnregister = $false

if ($vmProtectedDS.Count -gt 0) {
    # Re-check: are all datasources now in ProtectionStopped state?
    $activeRemaining = $vmProtectedDS | Where-Object {
        $_.properties.protectionState -ne "ProtectionStopped"
    }

    # Subtract the ones we just successfully stopped
    if ($dsToStop.Count -gt 0 -and $successCount -eq $dsToStop.Count) {
    ...
    if ($stillActiveCount -le 0) {
        Write-Host ""
        Write-Host "  All SAP HANA datasources on VM '$vmName' now have protection stopped." -ForegroundColor Green
        Write-Host ""
...
} elseif ($vmProtectedDS.Count -eq 0) {
    # No protected items found - VM might be registered but all datasources already unprotected
    Write-Host ""
    Write-Host "  No protected datasources found on VM '$vmName'." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "Summary:" -ForegroundColor Yellow
if ($vmProtectedDS.Count -gt 0 -and $dsToStop.Count -gt 0) {
    Write-Host "  Datasources processed:  $($dsToStop.Count)" -ForegroundColor White
